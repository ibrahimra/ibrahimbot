...
client.on('ready', () => {
  client.user.setGame(` |$help | By ibrahim.`,'https://www.twitch.tv/v5bz');
  console.log('---------------');
...
client.on('message', message => {
    ...
    if(message.content.includes('discord.gg')){
      if(!message.member.hasPermission('ADMINISTRATOR'))
        message.delete()
    return message.reply(`** ممنوع النفيات دير بالك لا تبلع ميوت :angry: ! **`)
    }
});
client.on('message', msg => {
...
client.on('message', msg => {
  if (msg.content === '$suppport') {
    msg.reply('السيرفر للمساعده,https://discord.gg/gJzUq5g');
  }
});
client.on("message", message => {
